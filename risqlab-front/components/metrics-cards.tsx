...
function MetricsCardsComponent({ indexData, globalData, fearGreedData, volatilityData }: MetricsCardsProps) {
  ...
  const renderVolatilityGauge = (volatilityPercent: number) => {
    ...
    return (
      <Card>
        <CardBody className="p-4">
          <div className="flex flex-col items-center gap-2">
            <span className="text-sm text-default-500 self-start">Volatility (90d)</span>
            <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
              {/* Green zone: 0-5% */}
              <path
                d={greenArc}
              ...
              <circle
                ...
                strokeWidth="3"
              />
              <circle cx={pointerX} cy={pointerY} r="4" className="fill-white dark:fill-gray-900" />
  ...
  return (
    <div className="flex flex-col gap-4 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {volatilityData?.current && renderVolatilityGauge(parseFloat(volatilityData.current.annualized_volatility) * 100)}
      </div>
    </div>
  );
}
