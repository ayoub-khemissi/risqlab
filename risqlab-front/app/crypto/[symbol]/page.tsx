...
import { TrendingUp, TrendingDown, ArrowLeft, Globe, FileText, Twitter, MessageCircle, Github } from "lucide-react";
import { CryptoDetailResponse } from "@/types/crypto-detail";
import { formatCryptoPrice, formatUSD, formatNumber, formatPercentage, getPercentageColor } from "@/lib/formatters";
...
export default function CryptoDetailPage() {
  ...
  const fetchCryptoDetail = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/cryptocurrency/${symbol.toUpperCase()}`);
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        <div className="text-lg text-danger">{error || "No data available"}</div>
  ...
  return (
    <section className="flex flex-col gap-6 py-8 md:py-10">
      ...
      <div className="flex items-center gap-4">
        {basic.logo_url && (
          <img src={basic.logo_url} alt={basic.name} className="w-16 h-16" />
        )}
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className={title()}>{basic.name}</h1>
            <Chip size="lg" variant="flat">{basic.symbol}</Chip>
            {market.cmc_rank && (
              <Chip size="sm" variant="bordered">Rank #{market.cmc_rank}</Chip>
            )}
          </div>
          {basic.category && (
      ...
      <Card>
        <CardBody className="p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <p className="text-sm text-default-500 mb-1">{basic.symbol} Price</p>
              <p className="text-4xl font-bold">{formatCryptoPrice(market.price_usd)}</p>
            </div>
              ...
              {market.percent_change_1h !== null && (
                <div>
                  ...
                  <Chip
                    ...
                    startContent={market.percent_change_1h > 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  >
              ...
              {market.percent_change_24h !== null && (
                <div>
                  ...
                  <Chip
                    ...
                    startContent={market.percent_change_24h > 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  >
              ...
              {market.percent_change_7d !== null && (
                <div>
                  ...
                  <Chip
                    ...
                    startContent={market.percent_change_7d > 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  >
              ...
              {market.percent_change_30d !== null && (
                <div>
                  ...
                  <Chip
                    ...
                    startContent={market.percent_change_30d > 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                  >
      ...
      {/* Links */}
      {(links.website || links.whitepaper || links.twitter || links.reddit || links.telegram || links.github) && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardBody className="p-4">
            ...
            <p className="text-2xl font-bold">{formatUSD(market.market_cap_usd)}</p>
          </CardBody>
        </Card>
        <Card>
          <CardBody className="p-4">
            <p className="text-sm text-default-500">Volume 24h</p>
            <p className="text-2xl font-bold">{formatUSD(market.volume_24h_usd)}</p>
          </CardBody>
        </Card>
        <Card>
          <CardBody className="p-4">
            <p className="text-sm text-default-500">Circulating Supply</p>
            <p className="text-2xl font-bold">{formatNumber(market.circulating_supply)} {basic.symbol}</p>
          </CardBody>
        </Card>
        {market.fully_diluted_valuation && (
          <Card>
            <CardBody className="p-4">
              <p className="text-sm text-default-500">Fully Diluted Valuation</p>
              <p className="text-2xl font-bold">{formatUSD(market.fully_diluted_valuation)}</p>
            </CardBody>
          </Card>
        )}
      </div>
          ...
          <CardBody className="p-6">
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">{formatNumber(market.circulating_supply)} {basic.symbol}</span>
              </div>
              {market.total_supply && (
                <div className="flex justify-between items-center">
                  <span className="text-sm text-default-500">Total Supply</span>
                  <span className="font-medium">{formatNumber(market.total_supply)} {basic.symbol}</span>
                </div>
              )}
              {market.max_supply && (
                <div className="flex justify-between items-center">
                  <span className="text-sm text-default-500">Max Supply</span>
                  <span className="font-medium">{formatNumber(market.max_supply)} {basic.symbol}</span>
                </div>
              )}
              {supplyPercentage > 0 && (
                <div>
                  ...
                  <Progress value={supplyPercentage} color="primary" />
                </div>
              )}
            </div>
          </CardBody>
      ...
      {basic.description && (
        <Card>
          ...
          <CardBody className="p-6">
            <p className="text-default-700 whitespace-pre-wrap">{basic.description}</p>
            <div className="mt-4 flex flex-wrap gap-4">
              ...
              {launch.date && (
                <div>
                  ...
                  <span className="text-sm font-medium">{new Date(launch.date).toLocaleDateString()}</span>
                </div>
              )}
            </div>
            {basic.tags.length > 0 && (
